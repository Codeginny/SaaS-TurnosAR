...
import { User, Settings, LogOut, Bell, BarChart3, Calendar, Shield, Building, MapPin, CreditCard, Users, ChevronDown, ChevronRight } from 'lucide-react';
...
const UserAvatar = () => {
  ...
  // Detectar nuevas notificaciones y activar animación
  useEffect(() => {
    const unreadCount = getUnreadCount();
    if (unreadCount > 0) {
      setShakeBell(true);
      const timer = setTimeout(() => setShakeBell(false), 1000);
  ...
  const formatTimestamp = (timestamp) => {
    // Validar que timestamp sea un Date válido
  ...
  return (
    <div className="relative flex items-center gap-4" ref={dropdownRef}>
      ...
      <div className="relative" ref={notificationsRef}>
        ...
        {isNotificationsOpen && (
          <div className="absolute top-full right-0 mt-2 w-80 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-gray-200 dark:border-slate-700 z-50 max-h-96 overflow-y-auto">
            <div className="p-4 border-b border-gray-100 dark:border-slate-700 bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-950/50 dark:to-blue-900/50">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-semibold text-gray-900 dark:text-white">Notificaciones</h3>
                  <p className="text-sm text-gray-600 dark:text-gray-300">
                    {notifications ? getUnreadCount() : 0} sin leer
                  </p>
                </div>
                {getUnreadCount() > 0 && (
                  <button
            ...
            <div className="p-2">
              {notifications && notifications.length > 0 ? (
                notifications.map((notification) => (
                  <div 
                    key={notification.id}
                    className={`notification-item p-3 rounded-lg mb-2 cursor-pointer ${
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {notification.action && notification.action !== 'none' && (
                          <p className="text-xs text-blue-600 dark:text-blue-400 mt-1 font-medium">
                            👆 Haz clic para {getActionText(notification.action)}
                          </p>
                        )}
                      </div>
                    </div>
            ...
            {notifications && notifications.length > 0 && (
              <div className="p-3 border-t border-gray-100 dark:border-slate-700 space-y-2">
                <button
                  onClick={markAllAsRead}
                  className="w-full text-center text-sm text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-medium py-2 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-950/30 transition-colors"
                >
                  Marcar todas como leídas
                </button>
