...
const adjustRGBOpacity = (rgbColor, opacity) => {
  // Regex to match rgb(r, g, b) or rgba(r, g, b, a)
  const match = rgbColor.match(/rgba?\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)(?:\s*,\s*[\d.]+)?\s*\)/);
  if (match) {
...
export const adjustColorOpacity = (color, opacity) => {
  ...
  if (color.startsWith('#')) {
    ...
    console.warn(`Unsupported color format: ${color}. Returning original color.`);
    return color;
  }
};
